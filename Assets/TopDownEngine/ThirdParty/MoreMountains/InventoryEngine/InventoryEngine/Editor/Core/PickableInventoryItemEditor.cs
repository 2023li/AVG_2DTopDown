		...
		public override void OnInspectorGUI()
		{
            ...
            if (ItemTarget.Item != null)
            {
                ...
                foreach (FieldInfo fi in type.GetFields())
                {
                    ...
                    {
                        // 检查LabelText特性
                        var labelAttribute = fi.GetCustomAttribute<MMLabelAttribute>();
                        string displayName = labelAttribute?.Label ?? fi.Name;

                        EditorGUILayout.LabelField(displayName, fi.GetValue(ItemTarget.Item).ToString());
                    }
                }
